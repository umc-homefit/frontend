...
/**
 * "만 원" 단위의 UI 입력 문자열을 "원" 단위 Long으로 변환
 * (백엔드 금액 필드는 모두 "원" 단위)
 *
 * 예: "4800" (4,800만 원) -> 48,000,000원
 *
 * 입력값이 비어있거나 숫자가 아니면 0을 반환
 */
 ...
 * 백엔드 monthlyIncomeAmount(월 총소득, 원 단위)로 변환
 *
 * "만 원" 단위에서 먼저 12로 나누면(예: 4800/12=400) 12로 안 떨어지는 값(예: 4000/12=333.33)에서
 * 소수점이 버려져 왕복 변환 시 오차가 커짐(4000만 원 입력 -> 3996만 원으로 보이는 문제)
 * 그래서 반드시 "원" 단위로 먼저 환산한 뒤 12로 나누고 반올림해서 오차를 최소화
...
/**
 * HouseStep 라디오 옵션 텍스트를 백엔드 housingOwnershipStatus enum 값으로 매핑
 *
 * - "완전 무주택" -> "HOMELESS"
 * - "본인 무주택, 세대원 유주택" -> "FAMILY_OWNED"
 * - "본인 유주택" -> "OWNED"
 *
 * 매칭되는 옵션이 없으면 "UNKNOWN"을 반환
 * "UNKNOWN"은 백엔드 enum에 존재하지 않는 방어용 값이므로 실제로 API 요청을 보내기 전에 호출부에서 별도 검증이 필요
 */
...
/**
 * housingOwnershipStatus 값으로부터 isHomeless(무주택 여부)를 파생시킴
 *
 * housingStatus가 "HOMELESS"(본인 및 세대원 모두 무주택)일 때만 true를 반환
...
/**
 * 백엔드 "원" 단위 금액을 "만 원" 단위 UI 표시 문자열로 변환 (toApiAmount의 반대 방향)
 *
 * 예: 48,000,000원 -> "4800" (4,800만 원)
 * 10,000으로 나눈 나머지(만 원 단위로 안 떨어지는 금액)는 버림 처리
 */
fun toDisplayAmount(apiAmount: Long): String = (apiAmount / 10_000).toString()

/**
 * 백엔드 monthlyIncomeAmount(월 총소득, 원 단위)를
 * IncomeStep의 "연간 총소득"(만 원 단위) 입력란에 채울 문자열로 변환 (toMonthlyIncomeAmount의 반대 방향)
 *
 * "원"을 먼저 "만 원"으로 내림 처리해버리면(예: 3,330,000/10,000=333) 그다음 12를 곱해도 원래 입력했던 값으로 못 돌아옴
 * 그래서 12를 먼저 곱해 "원" 단위로 정밀하게 계산한 뒤 맨 마지막에만 "만 원" 단위로 반올림
